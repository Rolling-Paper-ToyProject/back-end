...
CREATE TABLE IF NOT EXISTS `User` (  -- 테이블 이름에 대소문자 일관성 유지
    ...
    social_type VARCHAR(50) NOT NULL, -- EnumType.STRING으로 저장되는 socialType
...
CREATE TABLE IF NOT EXISTS `Student` (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT FK_roll_student FOREIGN KEY (roll_id) REFERENCES `Roll`(roll_id) ON DELETE CASCADE,
    UNIQUE KEY unique_student (name, pin_number)
    );
